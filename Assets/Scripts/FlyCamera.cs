...
public class FlyCamera : MonoBehaviour
{
    ...
    */
    ...
    private Vector3 GetBaseInput()
    {
        ...
        float xrot = ((float) Math.PI / 180) * this.transform.rotation.x;
        Double ymov = Math.Sin(Double.Parse(xrot.ToString()));
        Double zmov = Math.Cos(Double.Parse(xrot.ToString()));

        // Forwards
        if (Input.GetKey(KeyCode.W))
            p_Velocity += new Vector3(0, .5f, (float) zmov);
